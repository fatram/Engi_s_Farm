...
void Mixer::interact(Player& P){
	...
	if(bag.size() > 0){
		...
		for(int i = 0; i < P.bag.size(); i++){
			cout << (i+1) + ". " + P.bag.get(i).toString() << endl;
		}
		cout << "Pilih item yang akan digunakan (e.g 1 2 (Artinya item nomor 1 dan 2) : "
		int c1;
		int c2;
		cin >> c1;
		cin >> c2;
		if(c1 > P.bag.size() || c2 > P.bag.size()){
			cout << "Item yang dipilih tidak tersedia" << endl;
		} else {
			Product * p1 = P.bag.get(c1-1);
			Product * p2 = P.bag.get(c2-2);
			if(choice == 1){
